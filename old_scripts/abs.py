...
def replan_robot(element, man_dis, destination_path, time_, solve_):
	plans=[]
	file_name = "./solution/single/file_{}.lp".format(element)
	plans, time_, solve_ = plan(plans, element, man_dis, destination_path, time_, solve_)
	if plans:
...
def replan_parallel(robots_c, man_dis, destination_path, time, solve):
	pool = multiprocessing.Pool(processes=len(robots_c))
	results = [pool.apply_async(replan_robot, args=(r, man_dis, destination_path, time, solve)) for r in robots_c]
